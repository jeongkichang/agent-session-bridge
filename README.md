...
끝난 요청과 아무 기록도 걸리지 않은 오래된 세션 행은 기본 30일 뒤 지웁니다. 지우지 않으면 저장 파일이 계속 자랍니다(실측: 6일에 요청 955건, 5.7MB). 기간은 `AGENT_BRIDGE_RETENTION_DAYS`로 바꾸고, `0`을 주면 지우지 않습니다. 진행 중인 요청과, 기록이 가리키는 세션 행은 지우지 않습니다.

...
const tools = [
  ...
  { name: 'list_requests', description: 'List recent requests involving this connection, without message bodies. Check these states and get_reply before reporting that a decision or task reply is still pending. has_more means this is not a complete inventory. Previous connection IDs are not inherited.', inputSchema: { type: 'object', properties: { direction: { type: 'string', enum: ['sent', 'received', 'both'], default: 'both' }, limit: { type: 'integer', minimum: 1, maximum: 100, default: 20 } }, additionalProperties: false } },
...
const summaryInput = z.object({ direction: z.enum(['sent', 'received', 'both']).default('both'), limit: z.number().int().min(1).max(100).default(20) }).strict();

export function recoveryHint(code: string): string {
  if (code === 'peer_superseded') return 'A newer connection uses this peer name. Reconnect with a distinct name; do not take over or replay its work.';
  if (code === 'peer_offline') return 'Check list_peers for the current connection. A disconnected session is not automatically resumed. Read the original request before resending.';
  if (code === 'request_not_found') return 'This request is not available to this connection. After reconnecting, use the owner CLI get/history to inspect old requests; do not resend them as new work.';
  if (code === 'invalid_state') return 'Use get_reply for this exact request ID. Completed replies cannot be changed and delivery_unknown is not permission to rerun work.';
  if (code === 'invalid_input') return 'Check the tool schema, UUIDs and the 32 KiB UTF-8 text limit. Correct input before retrying.';
  if (code === 'not_found') return 'Check the broker version; newly added tools require an updated broker as well as an updated MCP connection.';
  return 'Read the request status before retrying any send. A timeout does not prove that execution failed.';
}
...
export async function runMcp(kind: 'claude' | 'codex') {
  ...
  const server = new Server({ name: 'session-bridge', version: VERSION }, {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request, extra) => {
    try {
      ...
      switch (request.params.name) {
        case 'list_peers': z.object({}).strict().parse(input); result = { self: { id: client.id, name, kind, version: VERSION }, broker: await client.request('/health'), peers: await client.peers() }; break;
        case 'list_requests': { const args = summaryInput.parse(input); result = await client.summaries(args.direction, args.limit); break; }
      ...
      return { isError: true, content: [{ type: 'text', text: JSON.stringify({ error: code, retry: recoveryHint(code) }) }] };
    }
  });
